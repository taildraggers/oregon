...
Daily aggregator of Oregon-located taildragger classified listings,
...
[Oklahoma](https://github.com/taildraggers/oklahoma) repos' approach,
with the state filter swapped to "OR".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, OR` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Oregon"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/oregon/`
...
```html
<iframe
  src="https://taildraggers.github.io/oregon/"
  title="Taildragger Ads in Oregon"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_OREGON_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
