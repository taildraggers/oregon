"""Aggregates Oregon-located listings from every taildraggers.com
...
ones whose location ends in ", OR".
...
repos' approach, with the state filter swapped to "OR".
"""
...
PAGE_TITLE = "Taildragger Ads in Oregon"
STATE_ABBR = "OR"
...
_OREGON_LOCATION_RE = re.compile(r",\s*OR\s*$", re.IGNORECASE)


def _is_oregon(location: str) -> bool:
    return bool(_OREGON_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        oregon_rows = [row for row in rows if _is_oregon(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(oregon_rows)} in Oregon")
        all_listings.extend(oregon_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Oregon listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Oregon listings: {len(listings)}")
